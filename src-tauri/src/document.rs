...
/// 検索 1 ヒット DTO（byte=pika-core の値・utf16=CM6 のハイライト/選択用）。
///
/// `pika-core::search` は UTF-8 バイトオフセットを返すが、CM6（フロントのエディタ）は
/// **UTF-16 コードユニット**で位置を扱う。frontend が CM6 のハイライト/選択にそのまま使えるよう、
/// command 境界で各マッチに UTF-16 オフセットを**追加**する（byte は残す＝二重に持つ）。
#[derive(Debug, Serialize)]
pub struct MatchDto {
    pub start: usize,
    pub end: usize,
...
pub fn search_in_text(
    ...
    // 各マッチに UTF-16 オフセットを付与する（CM6 は UTF-16 コードユニットで位置を扱う）。
    // マッチは前方走査で昇順（start_0 < end_0 ≤ start_1 …）なので `[start, end, start, end, …]` の
    // 昇順バイト列を作り、`byte_to_utf16_offsets` で **単一パス O(L)** で一括変換する
    // （per-match `encode_utf16().count()` は O(N·L) で禁止）。
    let mut byte_offsets = Vec::with_capacity(result.matches.len() * 2);
    ...
    let matches = result
        ...
        .map(|(i, m)| MatchDto {
            ...
            utf16_start: utf16[i * 2],
            utf16_end: utf16[i * 2 + 1],
        })
        .collect();
    Ok(SearchResultDto {
        matches,
...
/// 単一ヒット置換の結果 DTO（要件5.4「1件ずつ置換」・U5）。
///
/// `replaced=false`（`from` 以降にマッチなし）のときは `text` を元のまま返し、`start`/`end`/`next` は 0
/// （フロントは `replaced` を見てカーソル移動/通知を分岐する）。
#[derive(Debug, Serialize)]
pub struct ReplaceOneResultDto {
    pub text: String,
    pub replaced: bool,
    pub start: usize,
    pub end: usize,
...
/// 現在ヒット 1 件だけを置換する（要件5.4「1件ずつ置換」・U5）。
///
/// 置換計算（マッチ探索＋キャプチャ参照 `$1`/`${name}` の展開）は pika-core::search（cargo test 済み）。
/// `from` バイト位置以降の最初の 1 件を置換し、置換後の全文と元テキスト上の置換範囲、次検索開始位置を返す。
/// フロントでキャプチャ展開を実装しない（未テストの変換を TS に置かない＝設計核心判断1）。
#[tauri::command]
pub fn replace_one(
    text: String,
    query: String,
    ...
    from: usize,
    cancel: State<'_, SearchCancelService>,
) -> Result<ReplaceOneResultDto, String> {
    let token = cancel.begin();
    match search::replace_one(&text, &query, &replacement, options.into(), from, &token)
        .map_err(|e| e.to_string())?
    {
        Some(r) => Ok(ReplaceOneResultDto {
            text: r.text,
            replaced: true,
            start: r.start,
            end: r.end,
