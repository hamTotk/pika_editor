...
//! watcher イベント処理時は**内容ハッシュ一致を必須条件**として消し込む——
//! 現ディスク内容のハッシュが保存後ハッシュと一致する場合のみ自己イベントとして
//! **1 回だけ消費（ワンショット）**する。
//!
//! - **時刻窓は補助的な安全弁**。古いトークンの GC に使うだけで、窓を超過しても
//!   ハッシュが一致すれば自己保存として抑制する（リトライ・遅延書き込みを吸収）。
...
impl SaveTokenStore {
    ...
    /// 3. ハッシュ一致なら自己保存として抑制し、トークンを**ワンショットで消費**する
    ///    （窓を超過していても一致していれば抑制＝補助安全弁の時刻窓は抑制を妨げない）。
    pub fn decide(
        &mut self,
        path: &str,
        ...
        // 一致＝自己保存。ワンショット消費（次回同パスの変更は外部変更として通す）。
