...
//!
//! 配線状況（**デッドコードではない**）: 本層は要件9.3 由来の純粋ロジックで cargo test 済み。呼び出し側
//! （`SnapshotService` 永続化）の結線は系統C のリリース前繰り越し（`docs/acceptance.md` TD9「未実施・
//! 永続化実装後」）であり、結線前に削除すると検証済みの要件ロジックを失う。結線は別タスクで行う。
//!
//! 削除の適用順（要件9.3）:
