...
struct ReviewTarget
{
    ...
    // 単一 confirm（確認済み）でのみ使う「確定直前の再照合用ハッシュ」（LF 正規化 XXH3 hex）。
    // ユーザーが差分ビューで見た内容（差分新側＝エディタ内容）のハッシュ。confirm は確定直前に
    // 現ディスク内容（target.content）のハッシュと照合し、不一致なら見ていない内容のベースライン
    // 化を避けて中断する（要件8.3 / design 5.4 E2）。空のときは再照合しない（タブ未オープン等、
    // 見た内容の基準が無い経路では未設定でよい）。
