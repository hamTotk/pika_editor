...
- `bundle.windows.nsis.installMode = "currentUser"` … ユーザー単位インストール固定（管理者不要）。
  pika は per-user 設計＝関連付け（`HKCU`）もデータ（`%LOCALAPPDATA%`）もユーザー領域に置くため、
  全ユーザー（perMachine/HKLM）導入は提供しない（昇格アカウントにしか登録が効かず、アンインストール時の
  データ削除も実ユーザーへ届かないため）。別ユーザーで使うときはそのユーザーが自分でインストールする
...
- `bundle.resources`（base `tauri.conf.json`）… `THIRD_PARTY_NOTICES.txt` を同梱（About 画面は exe 埋め込み版を表示）
- `bundle.resources`（オーバーレイ `tauri.bundle.conf.json`）… 関連付け用の**種別アイコン** `icons/shell/*.ico`
  を INSTDIR へ同梱。`pika-cli --register-shell` が exe 隣から拾い、各 ProgId の `DefaultIcon` に書く
  （`pika-cli` の `resolve_icon_dir`。要件3.3・`src-tauri/icons/shell/README.md`）。`resources` は両ファイルとも
  **オブジェクト形式**なので `--config` の JSON Merge Patch で再帰マージされ、NOTICES と .ico が共存する（置換で
  落ちない）。実バンドル後は INSTDIR に NOTICES＋md.ico＋html.ico の3つが揃うことを系統C で目視確認する。
...
壊れる。よってバンドル専用設定 `src-tauri/tauri.bundle.conf.json`（`active:true` ＋ `externalBin` ＋ 種別アイコンの
`resources`）へ分離し、**バンドル時だけ** `--config` で深いマージして有効化する（単一ソースは保持＝NSIS は pika-cli を呼ぶ）。
