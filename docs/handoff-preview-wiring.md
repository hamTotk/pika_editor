...
## 検証ハーネス（前セッションで作成・流用可。`C:\Users\devuser\AppData\Local\Temp\`）
...
- フィクスチャ `C:\Users\devuser\pika-accept`: `showcase.md`（GFM 表/Mermaid/KaTeX/コード）・`f004-showcase.md`（Mermaid/KaTeX/highlight 総合）・`withscript.html`（JS あり＝系統B で無効化されるべき）・`selfcontained.html`・`external-res.md`（外部リソース opt-in）・`broken-refs.md`（欠落参照）・`img/sample.png`（ローカル画像）。
