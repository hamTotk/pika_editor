...
### 巨大ファイル読み取り
編集上限超は **Rust ストリーミングのバイト範囲読取＋フロントの仮想化ウィンドウビューア**（ログ/JSON ビューア型）で**読み取り専用**閲覧。CM6 に全量ロードしない。**range 配信は custom protocol（HTTP Range 様）か Channel API**（3章の IPCコストと一括で解決）。閲覧のみ（編集/保存/置換不可。第2段階の検索は MVP 外＝系統C 繰り越し・要件2.2 第2段階の精神。2026-06-25 改訂）。
