...
* **差分トグルとの両立（直交モデルの実装）**：差分の描画面は常にこの共有 WebView2 の差分HTML 1面とする（Scintilla にインライン差分を作らない＝軽量原則）。「プレビュー＋差分ON」は**左右grid で「左＝テキスト差分・右＝レンダリング（整形プレビュー）」を横並び**に描き、左右同時表示を実現する（左右は独立スクロール）。「ソース＋差分ON」は差分面のみ、「分割＋差分ON」は**プレビュー＋差分**（B11仕様変更。生ソースは出さず、上記「プレビュー＋差分ON」と同じ左右gridを流用。差分OFFで左エディタ＋右プレビューの編集分割へ復帰）。占有世代（4章）の切替軸に差分ON/OFFを含める（要件6.1・ui-design 8章）。**【2026-06-28 要件改定】** プレビュー＋差分の左右配置を「左＝レンダリング／右＝差分」から**「左＝差分／右＝レンダリング」へ入替**（Tauri 版は差分=DOM の `#diff-host`・レンダリング=権限ゼロ別WebView の `#preview-host` で別ホストに分かれており、grid-column で左=差分・右=プレビューに配置する。正典は ui-design 8章）
