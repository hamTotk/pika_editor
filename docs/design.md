...
* **差分トグルとの両立（直交モデルの実装）**：差分の描画面は常にこの共有 WebView2 の差分HTML 1面とする（Scintilla にインライン差分を作らない＝軽量原則）。「プレビュー＋差分ON」は**1枚の WebView2 内HTMLに左プレビュー・右差分を `grid` で横並び**に描き、1枚共有のまま左右同時表示を実現する（左右は独立スクロール）。「ソース＋差分ON」は差分面のみ、「分割＋差分ON」はエディタ（Scintilla）＋差分面。占有世代（4章）の切替軸に差分ON/OFFを含める（要件6.1・ui-design 8章）
